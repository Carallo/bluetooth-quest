...
export const CombatInterface = ({ characters, monsters, onBack, isNarratorMode = true, connectedDeviceId = null }: CombatInterfaceProps) => {
  ...
  const [isTargeting, setIsTargeting] = useState<{ attackerId: string | null; attackAction?: Action }>({ attackerId: null });
  ...
  const [attackModal, setAttackModal] = useState<{ open: boolean; participant: CombatParticipant | null }>({ open: false, participant: null });
  const [attackOptions, setAttackOptions] = useState<{ advantage: boolean, disadvantage: boolean }>({ advantage: false, disadvantage: false });
  ...
  const cancelTargeting = () => {
    ...
    toast({
      title: "Ataque cancelado",
      description: "Puedes seleccionar otra acción.",
    });
  };
  ...
  const handleAttack = (participant: CombatParticipant) => {
    setAttackModal({ open: true, participant });
  };

  const handleSelectTarget = (targetId: string, manualAdvantage: boolean, manualDisadvantage: boolean) => {
    ...
    let advantage = manualAdvantage;
    let disadvantage = manualDisadvantage;
    ...
    if (attackRoll >= hitThreshold) {
        let damage = rollDice(isTargeting.attackAction?.damage || '1d4');
        if (isCritical) damage += rollDice(isTargeting.attackAction?.damage || '1d4');
  ...
  const handleFlee = (participantId: string) => {
    const participant = participants.find(p => p.id === participantId);
    if (participant) {
      logAction(participantId, `${participant.name} ha huido del combate!`);
  ...
  return (
    ...
    <div className="min-h-screen bg-background p-4">
      ...
      <Dialog open={attackModal.open} onOpenChange={(open) => setAttackModal({ open, participant: null })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Realizar Ataque: {attackModal.participant?.name}</DialogTitle>
            <DialogDescription>
              Selecciona una acción y las condiciones del ataque.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {attackModal.participant?.type === 'character' && (attackModal.participant.data as Character).equipmentV2?.mainHand &&
              (() => {
                const weapon = (attackModal.participant.data as Character).equipmentV2.mainHand;
                const action: Action = {
                  name: weapon?.name || 'Ataque desarmado',
                  description: 'Ataque con arma principal',
                  damage: weapon?.damage || '1d4',
                };
                return (
                  <EpicButton onClick={() => {
                    setAttackModal({ open: false, participant: null });
                    setIsTargeting({ attackerId: attackModal.participant?.id || null, attackAction: action });
                    toast({ title: "Selecciona un objetivo", description: `Atacando con ${action.name}. Haz clic en el participante que deseas atacar.` });
                  }}>
            ...
            {attackModal.participant?.type === 'monster' && (attackModal.participant.data as Creature).actions?.map((action) => (
              <EpicButton key={action.name} onClick={() => {
                setAttackModal({ open: false, participant: null });
                setIsTargeting({ attackerId: attackModal.participant?.id || null, attackAction: action });
                toast({ title: "Selecciona un objetivo", description: `Atacando con ${action.name}. Haz clic en el participante que deseas atacar.` });
              }}>
                {action.name} ({action.damage || 'Sin daño'})
              </EpicButton>
            ))}
            <div className="flex items-center space-x-2">
              <Input type="checkbox" id="advantage" checked={attackOptions.advantage} onCheckedChange={(checked) => setAttackOptions(prev => ({ ...prev, advantage: !!checked, disadvantage: checked ? false : prev.disadvantage }))} />
              <Label htmlFor="advantage">Ventaja</Label>
            </div>
            <div className="flex items-center space-x-2">
              <Input type="checkbox" id="disadvantage" checked={attackOptions.disadvantage} onCheckedChange={(checked) => setAttackOptions(prev => ({ ...prev, disadvantage: !!checked, advantage: checked ? false : prev.advantage }))} />
              <Label htmlFor="disadvantage">Desventaja</Label>
            </div>
          </div>
        </DialogContent>
      </Dialog>
